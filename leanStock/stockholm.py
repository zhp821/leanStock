...
class Stockholm(object):
    ...
    def __init__(self, args):
        ...
        ##  --ktype {15,30,60}   分钟k线图 默认值:60
        self.ktype = args.ktype
    ...
    def load_all_quote_symbol(self):
        ...
        try:
            ...
            while (count < 100):
                ...
                print("from  "+r.url+"  download all stock code")
            ...
            print(u"Error: 下载股票编号失败..." + "\n")
            print(e)

        print(u"下载所有的股票编号完成... time cost: " + str(round(timeit.default_timer() - start)) + "s" + "\n")
        return all_quotes

    # 根据指定股票编号，下载此股票的60分和30分k数据
    def load_quote_min_data(self, quote, start_date, end_date, is_retry, counter,ktype):

        start = timeit.default_timer()

        if (quote is not None and quote['Symbol'] is not None):
            try:
                 dftmp = ts.get_k_data(quote['Symbol'],start_date,start_date,ktype)
                 quote_data=[]
                 for index, row in dftmp.iterrows():
                    onedayquote={}
                    onedayquote['Date'] = row['date']
                    onedayquote['Open'] = row['open']  #开盘
                    onedayquote['Close'] = row['close']  #收盘
                    onedayquote['High'] = row['high']  #最高
                    onedayquote['Low'] = row['low'] #最低
                    onedayquote['Volume'] = row['volume'] #成交量
                    quote_data.append(onedayquote)

                 quote['Data'] = quote_data
            except Exception as e:
                print (e)
                print(u"Error: 加载指定股票的数据失败... " + quote['Symbol'] + "/" + quote['Name'] + "\n")
                if (not is_retry):
                    time.sleep(2)
                    self.load_quote_min_data(quote, start_date, end_date, True,counter)  ##这里重试，以免是因为网络问题而导致的下载失败

            print(u"下载指定股票 " + quote['Symbol'] + "/" + quote['Name'] + " 完成..." + "\n")
    ...
    #根据股票符号，下载所有的股票数据
    def load_all_min_quote_data(self, all_quotes, start_date, end_date,ktype):
        print(u"开始下载所有股票数据..." + "\n")

        start = timeit.default_timer()

        counter = []
        mapfunc = partial(self.load_quote_min_data, start_date=start_date, end_date=end_date, is_retry=False, counter=counter,ktype=ktype)  #创建映射函数
        pool = ThreadPool(self.thread)  # 开辟包含指定数目线程的线程池
        pool.map(mapfunc, all_quotes)   # 多线程执行下载工作
        pool.close()
        pool.join()

        print(u"下载所有的股票数据完成... time cost: " + str(round(timeit.default_timer() - start)) + "s" + "\n")
        return all_quotes
    ...
    def data_process(self, all_quotes):
                ...
                try:
                    ...
                    for quote_data in quote['Data']:  #遍历当前股票每一天的数据
                        if(quote_data['Volume'] != '000' or quote_data['Symbol'] in self.index_array): #如果成交量不等于0或股票不是大盘指数才能正常处理
                            ...
                            #print("name:%s-code:%s-date:%s-Open:%10.3f-Close:%10.3f-High:%10.3f-Low:%10.3f" % (quote['Symbol'],d['Date'],d['Open'],d['Close'],d['High'],d['Low']))
                            temp_data.append(d)
        ...
        print(u"所有的股票处理结束... time cost: " + str(round(timeit.default_timer() - start)) + "s" + "\n")

    #数据导出
    def data_export(self, all_quotes, export_type_array, file_name):
        print(u"开始导出"+str(len(all_quotes))+"个股票数据")
        start = timeit.default_timer()
        ...
        if('json' in export_type_array):  #如果导出类型中包含json，就导出json文件
            ...
            json.dump(unicode(all_quotes), f, ensure_ascii=False)
        ...
        if('csv' in export_type_array):  #如果导出类型中包含csv，就导出csv文件
            ...
            if(all_quotes is not None and len(all_quotes) > 0):
              ...
              file_obj = open(directory + '/' + file_name + '.csv', 'wb')
              file_obj.write(codecs.BOM_UTF8) # 防止乱码
              writer = csv.writer(file_obj)
              #writer = csv.writer(io.open(directory + '/' + file_name + '.csv', 'wb'))  #使用指定的字符编码写入文件

              writer.writerow(columns)
            #writer.writerow(unicode(' '.join(columns)))  #写入列头作为第一行
                        ...
                        try:
                            ...
                            for column in columns:
                                if(column.find('data.') > -1):
                                    ...
                                    line.append(quote[column].encode('utf-8'))

                            if(len(line)>11 and line[9] is not None and line[7] is not None):
                                line.append(float(line[9])-float(line[7]))
                                line.append('http://finance.sina.com.cn/realstock/company/'+line[1]+'/nc.shtml')
        ...
        if('mongo' in export_type_array):
            print(u"开始导出到 MongoDB数据库...\n")

        print(u"导出数据完成.. time cost: " + str(round(timeit.default_timer() - start)) + "s" + "\n")
    ...
    def file_data_load(self):
        ...
        print(u"文件中数据加载"+str(len(all_quotes_data))+"个股票完成... time cost: " + str(round(timeit.default_timer() - start)) + "s" + "\n")
    ...
    def check_date(self, all_quotes, date):
        ...
        for quote in all_quotes:
            if(quote['Symbol'] in self.index_array):  #选取大盘数据，是因为大盘数据绝对会有，这就要求你爬虫时一定要把大盘数据获取下来
                for quote_data in quote[u'Data']:  #获取一只股票每一天的数据行情
                    if(quote_data[u'Date'] == date):   #爬虫的股票数据中的时间和将要测试的时间要相同
                        is_date_valid = True
    ...
    def quote_pick(self, all_quotes, target_date, methods):
        ...
        print(u"选股完成... time cost: " + str(round(timeit.default_timer() - start)) + "s" + "\n")
    ...
    def profit_test(self, selected_quotes, target_date):
        ...
        print(u"选股测试完成... time cost: " + str(round(timeit.default_timer() - start)) + "s" + "\n")
        return results

    #下载数据
    def data_load(self, start_date, end_date, output_types):
        all_quotes = self.load_all_quote_symbol()  #下载所有的股票符号
        print("all " + str(len(all_quotes)) + " stock download sucess..." + "\n")
    ...
    #下载分钟k线图数据
    def data_min_load(self,start_date, end_date,output_types,ktype):
        all_quotes = self.load_all_quote_symbol()  #下载所有的股票符号
        print("all " + str(len(all_quotes)) + " stock download sucess..." + "\n")
        self.load_all_min_quote_data(all_quotes, start_date, end_date,ktype)  #下载所有的股票数据
        self.data_process(all_quotes)  #数据处理
    ...
    def runmin(self):
        ...
        if(self.reload_data == 'Y'):
            print(u"开始下载股票数据...\n")
            self.data_min_load(self.start_date, self.end_date, output_types,self.ktype)